...
public class DeepEquals {
    ...
    private  final Map<Class, Collection<Field>> _reflectedFields = new ConcurrentHashMap<Class, Collection<Field>>();
    private  final Map<Class, Collection<String>> _ignoredFieldNames = new ConcurrentHashMap<Class, Collection<String>>();
    ...
    public  boolean deepEquals(Object a, Object b, List<String> excludedFieldNames) {
        _ignoredFieldNames.put(a.getClass(), excludedFieldNames);
     ...
     */
    private  boolean compareArrays(Object array1, Object array2, LinkedList stack, Set visited) {
     ...
     */
    private  boolean compareOrderedCollection(Collection col1, Collection col2, LinkedList stack, Set visited) {
        // Same instance check already performed...
     ...
     *         items will be added to the Stack for further comparison.
     */
    private  boolean compareUnorderedCollection(Collection col1, Collection col2, LinkedList stack, Set visited) {
        // Same instance check already performed...
     ...
     */
    private  boolean compareSortedMap(SortedMap map1, SortedMap map2, LinkedList stack, Set visited) {
        // Same instance check already performed...
     ...
     *         are equal, however, it will place the contents of the Maps on the stack for further comparisons.
     */
    private  boolean compareUnorderedMap(Map map1, Map map2, LinkedList stack, Set visited) {
        // Same instance check already performed...
     ...
     */
    public  boolean hasCustomEquals(Class c) {
        Class origClass = c;
     ...
     *         itself and just below Object in it's inheritance.
     */
    public  boolean hasCustomHashCode(Class c) {
        Class origClass = c;
     ...
     */
    public  Collection<Field> getDeepDeclaredFields(Class c) {
        ...
        Collection<Field> filteredFields = filterExcludedFields(c, fields);
        _reflectedFields.put(c, filteredFields);
        return filteredFields;
    }

    private  Collection<Field> filterExcludedFields(Class parentClass, Collection<Field> fields) {
        if (!_ignoredFieldNames.containsKey(parentClass)) {
            return fields;
        }

        Collection<String> classFieldsToIgnore = _ignoredFieldNames.get(parentClass);

        List<Field> fieldsToReturn = new ArrayList<Field>();
        for (Field field : fields) {
            if(!classFieldsToIgnore.contains(field.getName()))
